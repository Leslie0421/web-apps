    ...
    var _checkConfigParams = function () {
      if (_config.document) {
        ...
        if (
          ...
          console.warn(
            'The "documentType" parameter for the config object must take one of the values word/cell/slide/pdf.'
          );
        ...
        if (
          ...
          var type =
            /^(?:(xls|xlsx|ods|csv|gsheet|xlsm|xlt|xltm|xltx|fods|ots|xlsb|sxc|et|ett)|(pps|ppsx|ppt|pptx|odp|gslides|pot|potm|potx|ppsm|pptm|fodp|otp|sxi|dps|dpt)|(pdf|djvu|xps|oxps)|(doc|docx|odt|gdoc|txt|rtf|mht|htm|html|mhtml|epub|docm|dot|dotm|dotx|fodt|ott|fb2|xml|oform|docxf|sxw|stw|wps|wpt))$/.exec(
              _config.document.fileType
            );
          if (!type) {
            window.alert(
    ...
    (function (currentWindow) {
      ...
      function getGuid() {
        if (window.crypto && window.crypto.getRandomValues) {
          var uint = new Uint16Array(8);
          window.crypto.getRandomValues(uint);
          var count = 0;
          function getRandomValues() {
            return (65536 + uint[count++]).toString(16).substring(1);
          }
          return formatGuid(getRandomValues);
        }
        function getRandomValues() {
      ...
      function Connector(params) {
        ...
        this.guid = "asc.{" + getGuid() + "}";
        this.isConnected = false;
        this.callbacks = [];
        this.events = {};
        this.tasks = [];
        this.onMessageBound = this.onMessage.bind(this);
        params.autoconnect && this.connect();
        // 使用 Bsc ，避免和源码中的 Asc 冲突
        void 0 === window.Bsc && (window.Bsc = {});
        void 0 === window.Bsc.scope && (window.Bsc.scope = {});
      }
      Connector.prototype.onMessage = function (event) {
        if ("string" === typeof event.data) {
          var baseData = {};
          try {
            baseData = JSON.parse(event.data);
          } catch (error) {
            baseData = {};
          }
          if (
            "onExternalPluginMessageCallback" === baseData.type &&
            ((baseData = baseData.data), this.guid === baseData.guid)
          )
            switch (baseData.type) {
              case "onMethodReturn":
                0 < this.callbacks.length &&
                  (event = this.callbacks.shift()) &&
                  event(baseData.methodReturnData);
                0 < this.tasks.length && this.sendMessage(this.tasks.shift());
                break;
              case "onCommandCallback":
                0 < this.callbacks.length &&
                  (event = this.callbacks.shift()) &&
                  event(baseData.commandReturnData);
                0 < this.tasks.length && this.sendMessage(this.tasks.shift());
                break;
              case "onEvent":
                baseData.eventName &&
                  this.events[baseData.eventName] &&
                  this.events[baseData.eventName].call(
                    this,
                    baseData.eventData
                  );
            }
      ...
      Connector.prototype.sendMessage = function (task) {
        var message = {
        ...
        message.data = task;
        message.data.guid = this.guid;
        task = this.frame;
        "string" === typeof task &&
          (task = document.getElementById(this.frame));
        task && task.contentWindow.postMessage(JSON.stringify(message), "*");
      };
      Connector.prototype.connect = function () {
        if (this.isConnected) {
          console.warn("This connector is already connected");
        } else {
          if (window.addEventListener) {
      ...
      Connector.prototype.disconnect = function () {
        if (this.isConnected) {
          ...
          console.warn("This connector is already disconnected");
        }
      };
      Connector.prototype.callCommand = function (command, callback, isNoCalc) {
        if (this.isConnected) {
          this.callbacks.push(callback);
          command =
            "var Bsc = {}; Bsc.scope = " +
            JSON.stringify(window.Bsc.scope || {}) +
            "; var scope = Bsc.scope; (" +
            command.toString() +
            ")();";
          isNoCalc = {
          ...
          if (!this.callbacks.length) {
            this.tasks.push(isNoCalc);
          } else {
            this.sendMessage(isNoCalc);
          }
        } else {
          console.warn("Connector is not connected with editor");
        }
      };
      Connector.prototype.executeMethod = function (name, args, callback) {
        if (this.isConnected) {
          this.callbacks.push(callback);
          name = { type: "method", methodName: name, data: args };
      ...
      Connector.prototype.attachEvent = function (name, callback) {
        if (this.isConnected) {
          this.events[name] = callback;
          this.sendMessage({ type: "attachEvent", name });
        } else {
          console.warn("Connector is not connected with editor");
        }
      };
      Connector.prototype.detachEvent = function (name) {
        if (this.events[name]) {
          delete this.events[name];

          if (this.isConnected) {
            this.sendMessage({ type: "detachEvent", name: name });
          } else {
            console.warn("Connector is not connected with editor");
          }
        }
      };
      Connector.prototype._correctContextMenuItems = function (items) {
        for (
          var menuItems = { guid: this.guid, items: [] },
            formatItems = function (that, item) {
              var newItems = {
                id: getGuid(),
                text: item.text,
              ...
              if (item.items) {
                newItems.items = [];
                for (var l = 0, n = item.items.length; l < n; l++)
                  newItems.items.push(formatItems(that, item.items[l]));
              } else {
                if (item.onClick) {
                  that.contextMenuEvents[newItems.id] = true;
                  that.attachEvent(newItems.id, item.onClick);
                }
              }
              return newItems;
            },
          ...
          menuItems.items.push(formatItems(this, items[count]));
        return menuItems;
      };
      Connector.prototype.addContextMenuItem = function (items) {
        this.events.onContextMenuClick ||
          (this.events.onContextMenuClick = function (callback) {
            this.events[callback] && this.events[callback].call(this);
          }.bind(this));
        if (this.contextMenuEvents)
          for (var item in this.contextMenuEvents)
            this.contextMenuEvents.hasOwnProperty(item) &&
              this.detachEvent(item);
        this.contextMenuEvents = {};
        this.executeMethod("AddContextMenuItem", [
          this._correctContextMenuItems(items),
        ]);
      };
      Connector.prototype.updateContextMenuItem = function (items) {
        this.executeMethod("UpdateContextMenuItem", [
          this._correctContextMenuItems(items),
        ]);
      };
      currentWindow.Asc = currentWindow.Asc ? currentWindow.Asc : {};
      currentWindow.Asc.Connector = Connector;
    })(window);

    function _createConnector(settings) {
      return new Asc.Connector({
        frame: iframe,
  ...
  function getAppPath(config) {
    ...
    if (config.document) {
      if (typeof config.document.fileType === "string")
        type =
          /^(?:(pdf)|(djvu|xps|oxps)|(xls|xlsx|ods|csv|xlst|xlsy|gsheet|xlsm|xlt|xltm|xltx|fods|ots|xlsb)|(pps|ppsx|ppt|pptx|odp|pptt|ppty|gslides|pot|potm|potx|ppsm|pptm|fodp|otp)|(oform|docxf))$/.exec(
    ...
    if (type && typeof type[2] === "string") {
      // djvu|xps|oxps
      appType =
        config.type === "mobile" || config.type === "embedded" ? "word" : "pdf";
