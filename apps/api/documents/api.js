...
(function (DocsAPI, window, document, undefined) {
      ...
      Connector.prototype.addContextMenuItem = function (items) {
        ...
        if (this.contextMenuEvents)
          for (var item in this.contextMenuEvents)
            this.contextMenuEvents.hasOwnProperty(item) && this.detachEvent(item);
  ...
  function getAppPath(config) {
    ...
    if (typeof config.documentType === "string") {
      ...
      if (
        ...
        if (type && typeof type[1] === "string") app = appMap["pdf"];
      }
    } else if (
      !!config.document &&
      typeof config.document.fileType === "string"
    ...
    const path_type =
      config.type === "mobile"
        ...
        : config.document &&
          typeof config.document.fileType === "string" &&
          config.document.fileType.toLowerCase() === "oform"
        ? "forms"
        : "main";
  ...
  function getAppParameters(config) {
    ...
    var type = config.document
      ? /^(?:(pdf))$/.exec(config.document.fileType)
      : null;
    if (
